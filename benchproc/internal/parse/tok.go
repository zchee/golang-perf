...
// A tok is a single token in the kvql lexical syntax.
type tok struct {
...
// key returns the next key or operator token. A key may be a bare
// word or a quoted word.
func (t *tokenizer) key() (tok, tokenizer) {
	return t.next(false)
}

// value returns the next value or operator token. A value may be a
// bare word, a quoted word, or a regexp.
func (t *tokenizer) value() (tok, tokenizer) {
	return t.next(true)
}

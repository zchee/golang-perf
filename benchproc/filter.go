...
// filterFn is a filter function. If it matches individual values, it
// returns a non-nil mask (and the bool result is ignored). If it
...
// Apply rewrites res to keep only the Values that match the Filter f
// and reports whether any Values remain.
//
// Apply returns true if all or part of res is kept by the filter.
...
func (f *Filter) Match(res *benchfmt.Result) Match {
	m, x := f.match(res)
	return Match{len(res.Values), m, x}
}
...
// All reports whether all values in a result matched the query.
...
// Any reports whether any values in a result matched the query.
...
// Test reports whether value i matched the query.
...
// Apply rewrites res to keep only the Values that match m.

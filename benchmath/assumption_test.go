...
func checkSummary(t *testing.T, got, want Summary, warnings ...string) {
	...
	if got.Center != want.Center || got.Lo != want.Lo || got.Hi != got.Hi || got.Confidence != want.Confidence || !errorsEq(got.Warnings, want.Warnings) {
...
func checkComparison(t *testing.T, got, want Comparison, warnings ...string) {
	...
	if got.P != want.P || got.N1 != want.N1 || got.N2 != got.N2 || got.Alpha != want.Alpha || !errorsEq(got.Warnings, want.Warnings) {
		t.Errorf("got %#v, want %#v", got, want)
	}
}
